...
uses
  ...
  Vcl.ComCtrls, dxCore, cxDateUtils, cxCalendar, dxSkinMetropolis, dxSkinMetropolisDark, dxSkinOffice2013DarkGray,
...
type
  TfrmImportarXML = class(TForm)
    ...
    procedure btnLimparArquivosClick(Sender: TObject);
  ...
  private
    ...
    procedure SetCdsXML(Arquivo:string);
...
uses smGeral, untDMImportarXML, smMensagens, smDBFireBird, untDM, smDB, tissV2_02_01;
...
begin
  ...
  DiretorioNovo :=  ExtractFilePath(Application.ExeName) + Diretorio + '';
...
begin
  try
    DMImportarXML.XMLTransformProviderCabecalho.TransformRead.TransformationFile:='';
    ...
    DMImportarXML.XMLTransformProviderGuias.TransformRead.TransformationFile := '';
...
procedure TfrmImportarXML.btnLimparArquivosClick(Sender: TObject);
begin
  lstbxArquivos.Items.Clear;
  memoLog.Lines.Clear;
end;
...
begin
  ...
  try
    ...
    CopiarArquivo(ArquivoImportado,DiretorioApp + DiretorioImportando + '\' +  NomeArquivoImportado);

    //arquvio nao sera mais convertido
...
procedure TfrmImportarXML.CopiarArquivo(Origem, Destino: String);
var
  o, d: PWideChar;
begin
  o := PWideChar(Origem);
...
procedure TfrmImportarXML.SetCdsXML(Arquivo:string);
var
   MsgTiss :IXMLMensagemTISS;
   xmlGuias: IXMLCt_guiaSP_SADT;
   i, j: Integer;
begin
  try
    DMImportarXML.cdsXMLCabecalho.Close;
    DMImportarXML.cdsXMLCabecalho.CreateDataSet;

    DMImportarXML.cdsXMLGuias.Close;
    DMImportarXML.cdsXMLGuias.CreateDataSet;
  except on E:Exception do
    raise Exception.Create('Erro na abertura dos arquivos XML. ' + #13 + E.Message);
  end;


  MsgTiss := LoadmensagemTISS(Arquivo);

  DMImportarXML.cdsXMLCabecalho.Append;
  DMImportarXML.cdsXMLCabecalhoans_tipoTransacao.AsString:=MsgTiss.Cabecalho.IdentificacaoTransacao.TipoTransacao;
  DMImportarXML.cdsXMLCabecalhoans_sequencialTransacao.AsInteger:=MsgTiss.Cabecalho.IdentificacaoTransacao.SequencialTransacao;
  DMImportarXML.cdsXMLCabecalhoans_dataRegistroTransacao.AsString:=MsgTiss.Cabecalho.IdentificacaoTransacao.DataRegistroTransacao;
  DMImportarXML.cdsXMLCabecalhoans_horaRegistroTransacao.AsString:=MsgTiss.Cabecalho.IdentificacaoTransacao.HoraRegistroTransacao;
  DMImportarXML.cdsXMLCabecalhoans_registroANS.AsString:=MsgTiss.Cabecalho.Destino.RegistroANS;
  DMImportarXML.cdsXMLCabecalhoans_versaoPadrao.AsString:= MsgTiss.Cabecalho.VersaoPadrao;
  DMImportarXML.cdsXMLCabecalho.Post;




  for i := 0 to msgTiss.PrestadorParaOperadora.LoteGuias.Guias.GuiaFaturamento.GuiaSP_SADT.Count - 1 do
  begin
    xmlGuias:= msgTiss.PrestadorParaOperadora.LoteGuias.Guias.GuiaFaturamento.GuiaSP_SADT[i];

    for j := 0 to xmlGuias.ProcedimentosRealizados.Count-1 do
    begin
      DMImportarXML.cdsXMLGuias.Append;



      //Identificacao
      DMImportarXML.cdsXMLGuias.FieldByName('ans_registroANS').AsString:= xmlGuias.IdentificacaoGuiaSADTSP.IdentificacaoFontePagadora.RegistroANS;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_dataEmissaoGuia').AsString:= xmlGuias.IdentificacaoGuiaSADTSP.DataEmissaoGuia;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_numeroGuiaPrestador').AsString:= xmlGuias.IdentificacaoGuiaSADTSP.NumeroGuiaPrestador;

      //Dados do Beneficiario
      DMImportarXML.cdsXMLGuias.FieldByName('ans_numeroCarteira').AsString:= xmlGuias.DadosBeneficiario.NumeroCarteira;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_nomeBeneficiario').AsString := xmlGuias.DadosBeneficiario.NomeBeneficiario;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_nomePlano').AsString:= xmlGuias.DadosBeneficiario.NomePlano;

      //Dados do Solicitante
      DMImportarXML.cdsXMLGuias.FieldByName('ans_nomeContratado1').AsString:= xmlGuias.DadosSolicitante.Contratado.NomeContratado;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_nomeProfissional').AsString:= xmlGuias.DadosSolicitante.Profissional.NomeProfissional;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_siglaConselho').AsString:= xmlGuias.DadosSolicitante.Profissional.ConselhoProfissional.SiglaConselho;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_numeroConselho').AsString:= xmlGuias.DadosSolicitante.Profissional.ConselhoProfissional.NumeroConselho;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_ufConselho').AsString:= xmlGuias.DadosSolicitante.Profissional.ConselhoProfissional.UfConselho;

      //Prestador Executante
      DMImportarXML.cdsXMLGuias.FieldByName('ans_nomeContratado2').AsString:= xmlGuias.PrestadorExecutante.NomeContratado;
      //falta codigo prestador na operadora


      //Raiz
      DMImportarXML.cdsXMLGuias.FieldByName('ans_caraterAtendimento').AsString:= xmlGuias.CaraterAtendimento;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_tipoSaida').AsString:= xmlGuias.TipoSaida;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_tipoAtendimento').AsString:= xmlGuias.TipoAtendimento;


      //Procedimentos
      DMImportarXML.cdsXMLGuias.FieldByName('ans_data').AsString:= xmlGuias.ProcedimentosRealizados[j].Data;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_quantidadeRealizada').AsInteger:= xmlGuias.ProcedimentosRealizados[j].QuantidadeRealizada;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_valor').AsString := xmlGuias.ProcedimentosRealizados.Procedimentos[j].Valor;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_valorTotal1').AsString:= xmlGuias.ProcedimentosRealizados.Procedimentos[j].ValorTotal;

      DMImportarXML.cdsXMLGuias.FieldByName('ans_codigo').AsString:= xmlGuias.ProcedimentosRealizados.Procedimentos[j].Procedimento.Codigo;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_tipoTabela').AsString:= xmlGuias.ProcedimentosRealizados.Procedimentos[j].Procedimento.TipoTabela;
      DMImportarXML.cdsXMLGuias.FieldByName('ans_descricao').AsString:= xmlGuias.ProcedimentosRealizados.Procedimentos[j].Procedimento.Descricao;


      //Valor Total
      DMImportarXML.cdsXMLGuias.FieldByName('ans_valorTotal2').AsString:= xmlGuias.ValorTotal.TotalGeral;

      DMImportarXML.cdsXMLGuias.Post;
    end;

    if DMImportarXML.cdsXMLGuias.State in [dsInsert,dsEdit] then
      DMImportarXML.cdsXMLGuias.Post;
  end;
end;
...
begin
  DiretorioApp := PWideChar(ExtractFilePath(Application.Exename));
